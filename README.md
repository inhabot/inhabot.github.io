...
이미 Git에 추적된 비밀 파일은 `.gitignore`만 추가해도 사라지지 않습니다. 이
프로젝트는 새 저장소로 만들기 때문에 `.env.local`이 처음부터 추적되지 않도록
검증한 뒤 커밋해야 합니다. `git add -f .env.local`은 절대 사용하지 마세요.
...
`inhabot.github.io`는 정적 호스팅이므로 `server.mjs`를 실행하거나
`OPENAI_API_KEY`를 안전하게 보관할 수 없습니다. 이 저장소는 다음 방식으로
동작합니다.

- 로컬 Node 서버: `.env.local`의 키를 서버에서만 읽고 GPT 모드 사용
- GitHub Pages: 브라우저 안의 규칙 기반 공개 데모 모드로 즉시 사용
- 별도 API 서버 연결: GitHub 저장소의 `API_BASE_URL` Actions 변수에 백엔드
  주소를 설정하면 GPT 모드 사용

`main` 브랜치에 푸시하면 [`.github/workflows/pages.yml`](./.github/workflows/pages.yml)이
테스트, 비밀값 검사, 정적 파일 생성을 거쳐 `public` 폴더를 Pages에 배포합니다.
...
별도 GPT 백엔드를 배포할 때는 그 서비스의 Secret Manager에
`OPENAI_API_KEY`를 넣고 다음 환경 변수를 설정합니다.

```dotenv
ALLOWED_ORIGIN=https://inhabot.github.io
```

그 뒤 GitHub 저장소의 `Settings > Secrets and variables > Actions > Variables`에
공개 변수 `API_BASE_URL=https://별도-백엔드-주소`를 등록합니다. 이 변수에는 API
키가 아니라 백엔드의 공개 URL만 들어갑니다.
